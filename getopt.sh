...
do
    case "$OPT" in
        c )
            ...
            #     --group-id ${GroupID} \
            #     --protocol tcp --port 10443 --cidr 0.0.0.0/0
            ;;
...
echo 必ず実行される
#aws --region "$region" --profile "$profile" ec2  describe-security-groups --group-id ${GroupID} --output table
